class CarbonCalculator:
    ...
    def calculate(self, answers): 
        ...
        elif diet == 'lacto_vegetarian': food_val = 1.3 
...
#Transportation
        ...
        recommendations = []
        if diet in ['heavy_meat', 'regional_non_veg']:
            recommendations.append({
                'id': 'rec_food_diet', 'title': 'Incorporate More Plant-Based Days',
                ...
                'impact': 'Medium', 'estimatedReduction': 0.30, 'category': 'food',
            })
        if commute_mode in ['gas_car', 'two_wheeler'] and km_weekly > 100:
            recommendations.append({
                'id': 'rec_trans_public', 'title': 'Shift Commutes to Metro or Electric Public Transit',
                ...
                'impact': 'High', 'estimatedReduction': round(auto_val * 0.50, 2), 'category': 'transportation',
            })
        if ac_usage == 'heavy':
            recommendations.append({
                'id': 'rec_energy_ac', 'title': 'Set Air Conditioning to 24°C or Eco Mode',
                'description': 'Keeping your AC baseline at 24°C instead of 18°C reduces appliance electricity pull by up to 24%.',
                'impact': 'High', 'estimatedReduction': round(final_energy * 0.25, 2), 'category': 'energy',
            })
        if cooking_fuel == 'lpg':
            recommendations.append({
                'id': 'rec_energy_cooking', 'title': 'Transition to Electric Induction Cooking',
                'description': 'Pairing efficient electric induction cooktops lowers dependence on fossil LPG cylinders.',
                'impact': 'Low', 'estimatedReduction': 0.10, 'category': 'energy',
            })
